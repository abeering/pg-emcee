...
var validators = { 
	...
	single_key_numeric_value: function( command_split ){
		...
		if( !is_number( command_split[2] ) ){
			return { valid: false, error: 'bad command line format' };
		}
		return { valid: true };
	},
	storage_command: function( command_split, payload ){
		...
		if( payload && payload.length != command_split[4] ){
			// payload length doesn't match pre command bytes
			return { valid: false, error: 'bad data chunk' };
		}
		return { valid: true, payload_size: command_split[4] };
	},
};
