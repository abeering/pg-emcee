...
var conn_string = "tcp://" + this.config.db.user + ":" + this.config.db.pass + '@' + this.config.db.host + '/' + this.config.db.name;
var db_client = new pg.Client(conn_string);
db_client.connect();
...
// return expected size so server knows when to pass data on 
exports.requires_payload = function(command_line, conn){
	...
	if( memcachespeak.commands[command_split[0]] && memcachespeak.commands[command_split[0]].is_storage_command ){
		var validate_res = memcachespeak.commands[command_split[0]].validate( command_split ); 
		if( !validate_res.valid ){
			return false;
		}
		return validate_res['payload_size'];
	}
	return false;
};
